...
Treat repository rules and Accepted ADRs as review criteria. Treat the current
tree as authoritative; do not assume a product-specific module map or that a
documented future component has already been implemented.
...
- concurrent state changes without adequate synchronization,
- duplicate processing,
- non-idempotent retry behavior where retries are expected.

### Contract and compatibility integrity

Look for:

- public API, configuration, or persisted-format changes without compatible consumers or migration handling,
- boundary conversions that lose information or silently accept incompatible values,
- nondeterministic behavior where the repository contract requires reproducibility,
- changed assumptions that callers, artifacts, or external integrations do not validate.
...
- responsibilities or dependencies that contradict `ARCHITECTURE.md` or an applicable `AGENTS.md`,
- boundary violations, circular dependencies, or duplicated abstractions,
- a documented future architecture treated as already implemented,
...
- missing boundary or incompatibility cases for changed contracts.
